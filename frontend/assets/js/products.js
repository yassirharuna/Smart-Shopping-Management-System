...
(function () {
  ...
  function renderProductRows(products) {
    ...
    for (const p of list) {
      ...
      const image = p.image ? (p.image.startsWith('/') || p.image.startsWith('http') ? p.image : '/assets-img/' + p.image) : '';
      const imgHtml = image
        ? `<div class="product-image-wrap" data-id="${p._id}"><img class="product-image" src="${escapeHtml(image)}" alt="${escapeHtml(p.name)}"/></div>`
        : `<div style="width:40px;height:40px;border-radius:8px;background:var(--gray-200);"></div>`;
